...
(function(){
  ...
  function pickFamily(diff){
    ...
    if(diff === 'challenge') return E.pick(['order','abs_opp','order']);
    return E.pick(['order','compare','abs_opp']);
  }
  ...
  E.generateN703Engine = function(difficulty, questionType){
    ...
    const x = family==='order' ? E.pick(ORDER_SETS) : family==='compare' ? E.pick(COMPARE) : E.pick(ABS_OPP);
    let pts;
    if(family==='order') pts = x;
    else if(family==='compare') pts = [x.a,x.b];
    else pts = [x.n, x.opp];
    const svg = E.numberLineSvg({points:pts, min:-12, max:12});
