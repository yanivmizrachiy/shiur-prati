...
## Source-fit: coordinate system + bar chart — branch `claude/source-fit-coordinate-chart-v1` (2026-06-12) — DONE (scope)

- Finding addressed: generator was symbolic-heavy vs the sources' coordinate/graph/table/chart tasks (file 05 §N7-01, file 06 U-02/U-03, PATTERN_INDEX N7-01/N7-02).
- New diagrams.js helpers: E.coordinateGridSvg (Quadrant-I grid, axes+ticks+labels, labeled points A–D, optional polygon/polyline, highlighted axis-parallel segment, rectangle; collision-aware label offsets near edges) and E.barChartSvg (3–6 Hebrew categories, y-axis from zero with nice steps, optional values above bars, RTL category order, title strip). Both export-safe `<svg class="engine-svg">`, theme- and visual-mode compatible.
- New engine N7-01-ENGINE (pilot-n7-01.js) "מערכת צירים — רביע ראשון": plot_and_shape (rhombus per the source's A(4,1)B(2,3)C(4,5)D(6,3) family + rectangle/square/right-triangle), read_coordinate, segment_length_axis_parallel, rectangle_area_on_grid with challenge subs missing_vertex / area_to_coordinate ("שטח=…, מצאו קואורדינטה" from file 05). All 4 question types; difficulty per brief; TF truth-balanced; misconception distractors (swapped x/y, +1 point counting, added coordinates, perimeter-for-area). Edge-guards: area≠perimeter rectangles (4×4, 6×3), corner x≠y, segment lo≥1.
- U7-01 gained bar_chart_read family (pattern U-03 verbatim: value-by-category with values hidden so the scale must be read, highest/lowest, total) over 4 school-survey datasets with distinct counts; misconceptions: neighboring bar, first-bar/tallest without scale, max-as-total.
- Registration: pattern-engine.js (ENGINE_TOPIC_IDS, ENGINE_FN, TOPICS push, generators) + index.html script tag. No engine deleted or renamed; export.js untouched.
- Scope counters updated truthfully 25→26: harness-engines.mjs, release-audit.mjs, verify-phase2-static.mjs (requiredIds + activeSlices + status string), site-health.json, PROJECT_STATUS, SOURCE_ALIGNMENT (new N7-01 row). docs/SOURCE_FIT_DEEP_AUDIT_20260612.md created (was referenced but missing) with finding + remaining gaps.

## Source-fit visual expansion v2 — branch `claude/source-fit-visual-expansion-v2` (2026-06-12) — DONE (scope)

- U7-01 gained compare_groups_relative_frequency (file 06 pattern U-05): every generated case GUARANTEES the misconception trap — group A has more in absolute count but a lower rate, group B fewer but a higher rate; clean integer percentages enforced. open requires explanation via תדירות יחסית; mcq's distractor is always the larger-absolute group + "שווה"; tf balanced; mistake shows the absolute-count error and the answer computes both k/n. Visual: new E.doubleBarSvg (grouped bars n+k per group, values above bars, legend, RTL group order, y from 0).
