...
const requiredIds = ['G7-01','G7-02','G7-03','G7-04','N7-01','N7-03','N7-04','N7-05','N7-06','N7-07','A7-01','A7-02','A7-03','U7-01','U7-02','G8-01','G8-04','N8-01','N8-02','N8-03','N8-04','N8-05','A8-02','A8-03','U8-01','U8-02'];
...
if(health.activeSlices !== 26) throw new Error('site-health activeSlices must be 26');
...
const allJs = fs.readdirSync('generator').filter(f=>f.endsWith('.js')).map(f=>read('generator/'+f))
  .concat(fs.readdirSync('generator/engine').filter(f=>f.endsWith('.js')).map(f=>read('generator/engine/'+f))).join('\n');
...
if(!status.includes('Active generator slices (26)')) throw new Error('status missing 26 slices');
